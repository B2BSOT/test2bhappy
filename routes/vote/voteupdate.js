...
module.exports = function(app, connectionPool) {
    ...
    app.get('/vote/voteupdate', function(req, res, next) {
        ...
        connectionPool.getConnection(function(err, connection) {
            ...
            connection.query('select *, concat(left(vm.deadline,4),"/",substring(vm.deadline,5,2),"/",substring(vm.deadline,7,2))as deadlinedtShow'
                            ...
                            , [TEST_VOTE_ID], function(error, rows1) {

                if(error) {
                    connection.release();
                    throw error;
                }else {
                    connection.query('select * from vote_items where vote_id = ?;', [TEST_VOTE_ID], function(error, rows2) {
                        if(error) {
                            connection.release();
                            throw error;
                        }else {
            ...
            connection.beginTransaction(function(err) {
                ...
                connection.query('update vote_master '
                             ...
                             , [req.body.vote_contents, req.session.user_id, req.body.vote_deadline, req.body.parti_org_id, req.session.user_id, req.session.user_id, req.body.multi_yn,
                                req.body.secret_yn, req.body.add_item_yn, req.body.noti_yn, TEST_VOTE_ID], function(error, rows) {
                            if(error){
                                console.log("ERROR1");
